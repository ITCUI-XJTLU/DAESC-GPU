...
# =============================================================================
# DAESC-P2.sh - Individual Pipeline: Recombine FASTQ → CellRanger → SALSA
# =============================================================================
# Purpose:
...
if [[ -z "$root_path" || -z "$individual_id" || -z "$chr_list" ]]; then
    echo "❌ Usage: $0 root_path=... individual_id=... chr_list=..."
...
# ============================================================================
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║              Individual Pipeline Started                             ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ -f "${INDIV_DIR}/${SAMPLE_NAME}/outs/possorted_genome_bam.bam" ]]; then
    ...
    if [[ ! -f "${INDIV_DIR}/${SAMPLE_NAME}/outs/possorted_genome_bam.bam" ]]; then
        echo "❌ CellRanger failed! Check log: ${CELLRANGER_LOG}"
        exit 1
    fi
fi
...
# Steps: Genotype → Phase → Annotate → Filter → WASP → ASE Count
echo "═══════════════════════════════════════════════════════════════════════"
...
echo "🐋 Starting SALSA analysis in Singularity container..."
echo ""
...
for CHR in "${CHR_LIST[@]}"; do
    ...
    singularity exec \
        ...
        bash ${ROOT_PATH}/scr/salsa6_gatk_alleleCount.sh \
            ...
            --isphased \
            --threads 5 >> ${CHR_LOG} 2>&1
    echo "        ✅ Allele-specific counting done"
...
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║              Individual Pipeline Completed                           ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo ""
