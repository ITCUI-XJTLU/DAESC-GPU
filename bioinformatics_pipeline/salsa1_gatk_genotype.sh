...
# generate base recalibration table
  echo "Tengfei starts running gatk_germline_short_variant_scatter_gather "
  echo "Running BaseRecalibrator on scattered intervals for $interval"
  for scatter_interval in ${scatter_intervals[@]}; do
    gatk --java-options "-Xmx96G -XX:+UseParallelGC -XX:ParallelGCThreads=4" BaseRecalibrator \
      -I $1 \
  ...
  echo "scatter_interval: ${scatter_intervals[@]}"
  echo "scatter_intervals[0]: ${scatter_intervals[0]}"
  echo "scatter_intervals[1]: ${scatter_intervals[1]}"
  echo "scatter_intervals[2]: ${scatter_intervals[2]}"
  echo "scatter_intervals[3]: ${scatter_intervals[3]}"

  # 构建文件名
  recal_file="$intervaldir/recal_data.${scatter_intervals[0]}.table"
  # 检查文件是否存在并打印前几行
  if [ -f "$recal_file" ]; then
    echo "重新校准表的前几行："
    head -n 1 "$recal_file"
  else
    echo "文件 $recal_file 不存在。"
  fi
  ...
  echo "Tengfei is here: Running HaplotypeCaller on scattered intervals for $interval"
  for scatter_interval in ${scatter_intervals[@]}; do
    echo "$intervaldir/output.g.$scatter_interval.vcf.gz" >> /tmp/gvcf.list
    
    gatk --java-options "-Xmx96g -XX:+UnlockExperimentalVMOptions -XX:+UseCGroupMemoryLimitForHeap" HaplotypeCaller \
...
for interval in ${intervals[@]}; do
  ...
  # mapfile -t scatter_intervals < <(ls /tmp/interval_files_folder)
  scatter_intervals=($(ls /tmp/interval_files_folder/))
