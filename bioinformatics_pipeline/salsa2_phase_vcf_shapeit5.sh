...
echo "look at the shapeit4 file: "
ls $workdir/phasing/shapeit4/maps
...
echo "Tengfei 2"
echo "$workdir/phased.$interval.vcf.gz"
echo "$workdir/vcf.list"
echo "${intervals[@]}"
...
if [ $snvindel = "true" ]; then
  ...
  --region ${interval} \
  --reference $phasingref/phasing/biallelic_SNV/ALL.${interval}.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf.gz \
  --output $workdir/phased.${interval}.vcf.gz >> ${outputlog}
  done
elif [ $snvonly = "true" ]; then
  for interval in ${intervals[@]}; do
  echo "$workdir/phased.$interval.vcf.gz" >> $workdir/vcf.list
  $phasingref/shapeit5/SHAPEIT5_phase_common_static_v1.0.0 ${shapeit_args[@]} \
  --map $workdir/phasing/shapeit4/maps/${interval}.b38.gmap.gz \
  --region ${interval} \
  --reference $phasingref/phasing/biallelic_SNV/ALL.${interval}.shapeit2_integrated_v1a.GRCh38.20181129.phased.vcf.gz \
  --output $workdir/phased.${interval}.vcf.gz >> ${outputlog}
  
  bcftools view $workdir/phased.${interval}.vcf.gz | head -n 20
  done
fi
echo "Tengfei 2 End"
